...
class TestStatisticsAnalyst(SimpleTester):
    ...
    def _atom_test(self, period: int):
        dt1 = DateTime.now()
        # cython-sbt
        stat_future_period(arr=self._inputs, period=period)
        dt2 = DateTime.now()
        # numpy
        self._stat_future_period(arr=self._inputs, period=period)
        dt3 = DateTime.now()
        # python
        stat_future_period_py(arr=self._inputs, period=period)
        dt4 = DateTime.now()
        print("---------------------------------------")
    ...
    @classmethod
    def _stat_future_period(cls, arr: ndarray, period: int) -> ndarray:
