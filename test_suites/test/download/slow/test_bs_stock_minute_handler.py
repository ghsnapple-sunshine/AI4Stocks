...
class BsStockMinuteHandlerTest(BaseTest):
    def test_download_1_month(self) -> None:
        ...
        tbls = BsStockMinuteHandler(operator=self.operator).obtain_data(para=para)
    ...
    def test_download_1_year(self) -> None:
        ...
        tbls = BsStockMinuteHandler(operator=self.operator).obtain_data(para)
        assert stocks.shape[0] == len(tbls)
    ...
    def test_downlaod_in_a_day(self) -> None:
        ...
        hdl = BsStockMinuteHandler(operator=self.operator)
        # 正常下载数据
