...
class TestConnector(Tester):
    ...
    def _create_table(self):
        ...
        res = self._operator.execute(sql)
        assert res == 0

    def _insert_data(self):
        sql = "insert into `{0}` values(%s, %s, %s);".format(self._table_name)
        vals = [
            ["bravepuppy", "female", 18],
            ["ghsnapple", "male", 32],
            ["taylor", "female", 9],
        ]
        res = self._operator.execute_many(sql, vals, commit=True)
        assert res == 3

    def _select_data_cnt(self):
        sql = "select count(*) from `{0}`".format(self._table_name)
        res = self._operator.execute(sql)
        assert res == 1
    ...
    def _select_data2(self):
        sql = "select * from `{0}`".format(self._table_name)
        data = self._operator.execute(sql, fetch=True)
    ...
    def _delete_data(self):
        sql = "delete from `{0}` where name = 'ghsnapple'".format(self._table_name)
        res = self._operator.execute(sql, commit=True)
        assert res == 1

    def _drop_table(self):
        sql = "drop table if exists `{0}`".format(self._table_name)
        res = self._operator.execute(sql, commit=True)
        assert res == 0
