...
class SlowDownloadTest(Tester):
    def test_download(self):
            ...
            (StockDailyTask,),
            {
                ...
                "_args": (
                    Para().with_start_n_end(
                        start=Date(2020, 1, 1), end=Date(2022, 11, 1)
                    ),
                ),
                "_kwargs": {},
        ...
        assert self.operator.select_row_num("dc_stock_dayinfo_600000_hfq") == 684
        assert (
            self.operator.select_row_num("bs_stock_min5info_000001_") == 684 * 240 / 5
        )
        assert (
            self.operator.select_row_num("bs_stock_min5info_600000_") == 684 * 240 / 5
        )
