...
class StrategyMultiForks(bt.Strategy):
    ...
    def __init__(self):
        ...
        for i, d in enumerate(self.datas):
            self.inds[d] = dict()
            self.inds[d]["ma1"] = MovingAverageSimple(
                d.close, period=self.params.maperiod1
            )
            self.inds[d]["ma2"] = MovingAverageSimple(
                d.close, period=self.params.maperiod2
            )
            self.inds[d]["ma3"] = MovingAverageSimple(
                d.close, period=self.params.maperiod3
            )
            self.inds[d]["ma4"] = MovingAverageSimple(
                d.close, period=self.params.maperiod4
            )
            self.inds[d]["ma5"] = MovingAverageSimple(
                d.close, period=self.params.maperiod5
            )
            self.inds[d]["D1"] = CrossOver(
                self.inds[d]["ma5"], self.inds[d]["ma4"]
            )  # 交叉信号
            self.inds[d]["A1"] = CrossOver(
                self.inds[d]["ma1"], self.inds[d]["ma2"]
            )  # 交叉信号
