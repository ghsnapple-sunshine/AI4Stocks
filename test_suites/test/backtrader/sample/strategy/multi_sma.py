...
class Strategy(bt.Strategy):
    ...
    def notify_timer(self, timer, when, *args, **kwargs):
        ...
        # 只在5，9，11月的1号执行再平衡
        if self.data0.datetime.date(0).month in [1, 3, 5, 7, 9, 11]:
    ...
    def notify_order(self, order):
        ...
        if order._status in [order.Completed]:
            if order.isbuy():
                self.log(
                    "买单执行,%s, %.2f, %i"
                    % (order.df._name, order.executed.price, order.executed.size)
                )
    ...
    def notify_trade(self, trade):
        if trade.isclosed:
            print(
                "毛收益 %0.2f, 扣佣后收益 % 0.2f, 佣金 %.2f, 市值 %.2f, 现金 %.2f"
                % (
                    trade.pnl,
                    trade.pnlcomm,
                    trade.commission,
    ...
    def rebalance_portfolio(self):
        ...
        self.log(
            "下单, 标的个数 %i, targetvalue %.2f, 当前总市值 %.2f"
        ...
        for d in self.ranks:
            ...
            size = int(
                abs(
                    (self.broker.getvalue([d]) - targetvalue)
                    / d.initial[1]
                    // 100
                    * 100
                )
