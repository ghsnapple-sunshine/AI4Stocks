...
from buffett.common.magic import (
    TYPE,
    MTD,
    BM,
    OBJ,
...
class SerializeTool:
    _SERIALIZE = {
        (int, float, bool, str, bytes, NoneType): lambda o, c: o,
        (list, tuple, set): lambda o, c: {
            TYPE: get_name(c),
            OBJ: [SerializeTool._do(v) for v in o],
        },
        (dict,): lambda o, c: {
            TYPE: DICT,  # 等效于 getattr(c, NAME)
            OBJ: {}
            if len(o) == 0
            else dict(
                [(SerializeTool._do(k), SerializeTool._do(v)) for k, v in o.items()]
            ),
        },
        (complex,): lambda o, c: {
            TYPE: CMPLX,  # 等效于 getattr(c, NAME)
            OBJ: [o.real, o.imag],
        },
    }
    _SERIALIZE = dict(
    ...
    _DESERIALIZE = {
        LIST: lambda o: [SerializeTool._undo(v) for v in o],
        TUPLE: lambda o: (SerializeTool._undo(v) for v in o),
        SET: lambda o: (SerializeTool._undo(v) for v in o),
        DICT: lambda o: dict(
            [(SerializeTool._undo(k), SerializeTool._undo(v)) for k, v in o.items()]
        ),
    ...
    def _do(obj):
        ...
        # 其他object类型
        otp = {TYPE: get_name(obj_cls), SOURCE: get_module_name(obj_cls)}
        otp_obj = {}
        for att_name in dir(obj):
            ...
            if (
                hasattr(obj_cls, att_name) and getattr(obj_cls, att_name) == obj_attr
            ):  # 不处理类属性
                continue
    ...
    def _undo(source, target):
        ...
        if SOURCE not in source:
            return (
                {}
                if len(source) == 0
                else SerializeTool._DESERIALIZE[source[TYPE]](source)
            )
