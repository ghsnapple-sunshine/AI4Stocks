...
class CreateSqlTools:
    ...
    @staticmethod
    def create(name: str, meta: DataFrame, if_not_exist=True) -> str:
        """
        create table
        """
        ...
        if not prim_key.empty:
            apd = "primary key ({0})".format(",".join([f"`{x}`" for x in prim_key]))
            cols.append(apd)
    ...
    @staticmethod
    def alter(name: str, curr_meta: DataFrame, new_meta: DataFrame) -> Optional[str]:
        """
        alter table
        """
        comb_meta = pd.merge(
            curr_meta, new_meta, how="outer", on=[COLUMN], suffixes=["_l", ""]
        )
        ...
        if len(diffs) == 0:
            return None
        sql = "alter table `{0}` {1}".format(name, ",".join(diffs))
        return sql
