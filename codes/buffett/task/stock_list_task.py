...
class StockListTask(Task):
    def __init__(self,
        ...
        super().__init__(wrapper=Wrapper(StockListHandler(operator=operator).obtain_data),
                         start_time=start_time)
        self._operator = operator

    def get_subsequent_task(self, success: bool):
        if success:
            return StockListTask(operator=self._operator,
                                 start_time=self._start_time.add(days=15))
        else:
            return StockListTask(operator=self._operator,
