...
from ai4stocks.common import DataSourceType as SouType, DataFreqType as FreqType, FuquanType as FType, StockCode as Code
from ai4stocks.common.tools import timestamp_to_datetime as t2d
from ai4stocks.download.connect import MysqlOperator as Op
from ai4stocks.download.fast.stock_list_handler import StockListHandler as StockListHandler
from ai4stocks.download.handler_base import HandlerBase as Handler, HandlerParameter as Para
from ai4stocks.download.slow.download_recorder import DownloadRecorder as Rcd


# region 静态方法
def __handle_datetime__(
        start_time: DateTime,
        end_time: DateTime
) -> tuple:
    ntz_start_time = __simplify_datetime__(time=start_time)
    ntz_end_time = __simplify_datetime__(time=end_time)
    ntz_last = __simplify_datetime__(
        time=DateTime.now(),
        to_day=True
    ) - Duration(minutes=1)
...
class SlowHandlerBase(Handler):
    def __init__(self, operator: Op):
        super(SlowHandlerBase, self).__init__(operator=operator)
        self.recorder = Rcd(op=operator)
        self.source = SouType.AKSHARE_DONGCAI
        self.fuquans = [FType.NONE]
        self.freq = FreqType.DAY
...
# region 公共方法
    def download_and_save(
        ...
        start_time, end_time = __handle_datetime__(
            start_time=start_time,
            end_time=end_time
        )
        stocks = StockListHandler(self.operator).get_table()
    ...
    def obtain(self, para: Para):
        return self.download_and_save(
            start_time=para.datespan.start.to_datetime(),       # TODO:整改Handler中的传参类型
            end_time=para.datespan.end.to_datetime())
    ...
    def get_table(
        ...
        table_name = self.__get_table_name__(
            code=code,
            fuquan=fuquan)
        return self.operator.get_table(table_name)
...
# region 私有方法
    ...
    def __download_partial_info__(
        ...
        cur_start_time = t2d(record['start_date'].iloc[0])
        cur_end_time = t2d(record['end_date'].iloc[0])
